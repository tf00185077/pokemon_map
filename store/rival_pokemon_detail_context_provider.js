...
import { RivalPokemonDetailContext } from './rival_pokemon_detail_context'
...
export const RivalPokemonDetailProvider = (props) => {
  const pokemonData = useContext(PokeDataContext)
  const { choose } = useContext(ChooseNoContext)
  const chooseNumber = choose['rival'] //找出自己context的編號
  ...
  // console.log(defaultValue,"Rival")
  return (
    <RivalPokemonDetailContext.Provider value={defaultValue}>
      {props.children}
    </RivalPokemonDetailContext.Provider>
  )
}
